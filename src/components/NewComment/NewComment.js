...
const NewComment = () => {
  ...
  const addCommentHandler = (k) => {
    ...
    axios
      .post("http://localhost:5000/experience/:id/comment", newComment)
      .then((response) => {
  ...
  return (
    <Form className="NewComment" onSubmit={addCommentHandler}>
      ...
      {commentSent.disableAlert ? (
        ...
        <Alert
          variant={commentSent.commentConfirm === true ? "success" : "danger"}
        >
          Comment{" "}
          {commentSent.commentConfirm === true
            ? "Shared"
            : "not Shared, please check your network."}
        </Alert>
      )}
    </Form>
  );
